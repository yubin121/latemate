...
`~/.claude/CLAUDE.md`(전역)에 작성되어 있으며, 이 파일에는 중복 기술하지 않는다.

```
~/.claude/CLAUDE.md        ← 전역: 워크플로우, 코드 스타일, 브랜치 전략 등 공통
└── latemate/CLAUDE.md     ← 프로젝트 특화: 기술 스택, 폴더 구조, 명령어, 개발 제약
```
...
> 네이밍, 컴포넌트 원칙, props 구조 분해 등 범용 규칙은 `~/.claude/CLAUDE.md` 참조.
