...
import { Navigation, Clock, AlertCircle, ArrowRight, MapPin } from 'lucide-react'
...
function PhoneFrame({ children }: { children: React.ReactNode }) {
  return (
    <div
      ...
      style={{ width: 220, filter: 'drop-shadow(0 32px 64px rgba(0,0,0,0.25))' }}
    >
      <div
      ...
      >
        ...
        <div className="absolute inset-[7px] bg-white rounded-[36px] overflow-hidden">
          ...
          {/* Status bar */}
          <div className="absolute top-0 inset-x-0 flex items-center justify-between px-5 z-20" style={{ height: 44 }}>
            <span className="text-[10px] font-semibold text-gray-800">9:41</span>
            <div className="flex items-center gap-1">
              <svg width="14" height="10" viewBox="0 0 14 10" fill="none">
                <rect x="0"    y="4"   width="2.5" height="6"   rx="0.5" fill="#1f2937" />
                <rect x="3.5"  y="2.5" width="2.5" height="7.5" rx="0.5" fill="#1f2937" />
                <rect x="7"    y="1"   width="2.5" height="9"   rx="0.5" fill="#1f2937" />
                <rect x="10.5" y="0"   width="2.5" height="10"  rx="0.5" fill="#1f2937" opacity="0.3" />
              </svg>
  ...
  return (
    <div className="h-full flex flex-col bg-[#bacde0]">
      ...
      <div className="bg-[#f6f6f6] flex items-center gap-2 px-3 py-2 border-b border-gray-200 flex-shrink-0">
        ...
        <div className="w-5 h-5 rounded-full bg-yellow-400 flex items-center justify-center text-[7px] font-bold text-yellow-900">3</div>
        <div>
          <p className="text-[9px] font-bold text-gray-800 leading-none">이번 주 약속</p>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="bg-white flex flex-col items-center px-3 py-2 shadow-sm flex-shrink-0">
        ...
        <p className="text-[8px] text-brand-500 mt-0.5">오늘 18:00 · 강남역 1번 출구</p>
      </div>
      {/* Map */}
      <div className="flex-1 relative overflow-hidden bg-[#e8f0e9]">
        {/* Roads */}
        <div className="absolute left-0 right-0 h-[10px] bg-white opacity-60" style={{ top: '44%' }} />
        <div className="absolute top-0 bottom-0 w-[10px] bg-white opacity-60" style={{ left: '46%' }} />
        {/* Grid */}
        {[15, 30, 60, 78].map((p) => (
          <div key={p} className="absolute left-0 right-0 border-t border-[#d0e6d0] opacity-60" style={{ top: `${p}%` }} />
        ))}
        {[22, 65].map((p) => (
          <div key={p} className="absolute top-0 bottom-0 border-l border-[#d0e6d0] opacity-60" style={{ left: `${p}%` }} />
        ))}
        {/* Green patches */}
        <div className="absolute w-10 h-8 bg-green-200 rounded-lg opacity-80" style={{ top: '8%', left: '8%' }} />
        <div className="absolute w-8 h-9 bg-green-200 rounded-lg opacity-80" style={{ bottom: '12%', right: '8%' }} />

        {/* Destination pin */}
        <div className="absolute z-20 -translate-x-1/2 -translate-y-full" style={{ top: '45%', left: '50%' }}>
      ...
      <div className="bg-white shadow-[0_-4px_16px_rgba(0,0,0,0.08)] flex-shrink-0 px-3 pt-2 pb-3 rounded-t-2xl">
        ...
        {[
          ...
          <div key={p.name} className="flex items-center justify-between py-[3px]">
            <div className="flex items-center gap-1.5">
              <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[7px] font-bold ${p.late ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-700'}`}>
                {p.name[0]}
              </div>
              <span className="text-[9px] font-semibold text-gray-800">{p.name}</span>
            </div>
            <span className={`text-[7px] rounded-full px-2 py-0.5 font-semibold ${p.late ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-700'}`}>
...
export default function LandingPage() {
  ...
  useEffect(() => {
    const onScroll = () => setShowStickyBar(window.scrollY > 500 && !bottomCtaInView.current)
    ...
    <div className="flex flex-col">
      ...
      <section className="min-h-dvh flex flex-col bg-white px-6 pt-14 pb-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-2.5">
          <img
            src="/latemate_app_icon.svg"
            alt="LateMate"
            className="w-9 h-9 rounded-xl object-cover flex-shrink-0"
          />
          <span className="text-lg font-bold text-brand-700 tracking-tight">LateMate</span>
        </div>
        ...
        <div className="relative z-10 flex justify-center">
          <div className="flex flex-col items-center gap-1.5 opacity-35">
            ...
            <p className="text-[9px] text-gray-400 tracking-[0.2em] font-medium">SCROLL</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-gray-50 px-6 py-16">
        <FadeUp>
          <h2 className="text-[26px] font-black text-gray-900 mb-8 text-center">핵심 기능</h2>
        </FadeUp>
          ...
          {[
            ...
            <FadeUp key={f.title} delay={i * 100}>
              <div className="bg-white rounded-2xl px-4 py-5 shadow-sm flex items-start gap-4">
                <div className={`w-11 h-11 rounded-2xl flex-shrink-0 flex items-center justify-center ${f.bg} ${f.color}`}>
                  {f.icon}
                </div>
                <div>
      ...
      <section className="bg-white px-6 py-16">
        <FadeUp>
          <h2 className="text-[26px] font-black text-gray-900 mb-10 text-center">3단계로 끝</h2>
        </FadeUp>
        <div className="flex flex-col">
          {[
            { step: '01', title: '약속 만들기', desc: '제목, 장소, 시간을 입력해 약속을 생성해요.' },
            { step: '02', title: '링크 공유',   desc: '카톡으로 1초 전송. 친구들이 클릭만 하면 돼요.' },
            { step: '03', title: '실시간 확인', desc: '지도에서 모두의 위치와 도착 예정 시간을 확인해요.' },
          ].map((s, i) => (
            <FadeUp key={s.step} delay={i * 120}>
              <div className="flex gap-5 mb-8 last:mb-0">
                <div className="flex flex-col items-center">
                  ...
                  {i < 2 && (
                    <div className="w-px flex-1 mt-2 min-h-8" style={{ background: 'linear-gradient(to bottom, #c7d2fe, transparent)' }} />
                  )}
                </div>
      ...
      {/* ── Bottom CTA ── */}
      <section ref={bottomCtaRef} className="px-6 py-16 text-center relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #4F46E5, #4338CA)' }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at top right, rgba(99,102,241,0.6), transparent 60%)' }} />
        <FadeUp className="relative z-10">
          <p className="text-brand-200 text-sm mb-2">회원가입 없이 바로 시작</p>
          <h2 className="text-[28px] font-black text-white mb-2">30초면 충분해요</h2>
          <p className="text-brand-200 text-sm mb-8">실시간 위치 공유, 지금 바로 시작해요.</p>
          <button
            onClick={() => navigate('/create')}
