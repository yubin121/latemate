...
export default function JoinPage() {
  ...
  const { mutate: join, isPending, error: joinError } = useJoinAppointment(appointmentId!)
  ...
  // joinError 처리
  const errorMessage =
    joinError instanceof Error ? joinError.message : joinError ? '참여에 실패했어요.' : ''
  ...
  if (isError || !appointment) {
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh px-6 gap-4">
        <p className="text-base font-semibold text-gray-700">약속을 찾을 수 없어요</p>
        <Button variant="secondary" onClick={() => navigate('/')}>홈으로 이동</Button>
      </div>
    )
  }

  const isExpired = appointment.expires_at && new Date(appointment.expires_at) < new Date()
  if (isExpired) {
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh px-6 gap-4">
        <p className="text-base font-semibold text-gray-700">종료된 약속이에요</p>
        <Button variant="secondary" onClick={() => navigate('/')}>홈으로 이동</Button>
      </div>
    )
  }
    ...
    <div className="flex flex-col min-h-dvh bg-surface-page">
      <div className="flex-1 px-4 pt-8 pb-32">
        ...
        <div className="bg-white rounded-2xl shadow-card overflow-hidden mb-5">
          ...
          <div className="p-5">
            <h1 className="text-xl font-bold text-gray-900 mb-3">{appointment.title}</h1>
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-1.5">
              <MapPin size={14} strokeWidth={1.5} className="text-brand-500 flex-shrink-0" />
              <span>{appointment.place_name}</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <Clock size={14} strokeWidth={1.5} className="text-brand-500 flex-shrink-0" />
        ...
        <div className="bg-white rounded-2xl p-4 shadow-card">
          ...
          {(nicknameError || errorMessage) && (
            <p className="mt-1.5 text-xs text-status-late">{nicknameError || errorMessage}</p>
          )}
